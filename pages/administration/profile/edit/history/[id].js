...
export default function () {
  ...
  const [edu_background_degree, setEduBackgroundDegree] = useState("");
  const [edu_background_name	, setEduBackgroundName] = useState("");
  const [edu_background_faculty, setEduBackgroundFaculty] = useState("");
  const [edu_background_departement, setEduBackgroundDepartement] = useState("");
  const [edu_background_major, setEduBackgroundMajor] = useState("");
  useEffect(() => {
    getStudentData();
  }, [id]);
      ...
      try {
        ...
        console.log(`🚀 ~ file: [id].js ~ line 80 ~ getStudentData ~ data`, data)
  			...
  			setStatus(data.data.status);
				...
				setEduBackgroundDegree(data.data.edu_background_degree)
				setEduBackgroundName(data.data.edu_background_name	)
				setEduBackgroundFaculty(data.data.edu_background_faculty)
				setEduBackgroundDepartement(data.data.edu_background_departement)
				setEduBackgroundMajor(data.data.edu_background_major)
      } catch (error) {
        if (error.response) {
	...
	async function submitStudent() {
		try {
			const sendData = {
				...
				campus_departement,
				campus_end,
      	edu_background_degree	,
				edu_background_name	,
				edu_background_faculty ,
				edu_background_departement ,
				edu_background_major,
			}	
			let prepareData = {
				...studentData,
				user_info : {
  ...
  return (
    <FormLayout title="Student Edit | AIS University" titlePage="Student History Edit">
			<Grid
				container
				spacing={1}
				direction="row"
      ...
      >
        ...
        <FormContainer
          label="Degree"
          name="edu_background_degree"
          value={edu_background_degree}
          setValue={setEduBackgroundDegree}
					width="95%"
        />
        <FormContainer
          label="Name of University"
          name="edu_background_name"
          value={edu_background_name}
          setValue={setEduBackgroundName}
					width="95%"
        />
        <FormContainer
          label="School or Faculty"
          name="edu_background_faculty"
          value={edu_background_faculty}
          setValue={setEduBackgroundFaculty}
					width="95%"
        />
        <FormContainer
          label="Department"
          name="edu_background_departement"
          value={edu_background_departement}
          setValue={setEduBackgroundDepartement}
					width="95%"
        />
        <FormContainer
          label="Major"
          name="edu_background_major"
          value={edu_background_major}
          setValue={setEduBackgroundMajor}
					width="95%"
        />
        {/* <FormContainer
          label="Date Of Birth"
