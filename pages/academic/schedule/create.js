...
export default function () {
  ...
  const [end_time, setEndTime] = useState(moment().add(50, 'minutes'));
	...
	async function getTeacher() {
		if(departement_id != '')
			try {
				const { data } = await axios.get('/api/teacher?faculty_id='+faculty)
  ...
  return (
    ...
    <FormLayout title="Course Schedule Create | AIS University" titlePage="Course Schedule Create">
      <Stack
        mb={4}
        sx={{
          width: 640,
