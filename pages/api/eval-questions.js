...
const EvalQuestions = require("../../models/masterEvalQuestions")(
  db.sequelize,
  DataTypes
);
...
export default nextConnect()
  ...
  .post(isAdmin, async (req, res) => {
    ...
    try {
      ...
      const data = await EvalQuestions.create(body);
  ...
  .get(async (req, res) => {
    if (req.query.id) {
      try {
        const data = await EvalQuestions.findOne({
      ...
      try {
        const data = await EvalQuestions.findAll();
        if (data.length == 0)
  ...
  .patch(isAdmin, async (req, res) => {
    ...
    delete body.id;
    try {
      const data = await EvalQuestions.update(body, {
        where: { id: id },
      });
  ...
  .delete(isAdmin, async (req, res) => {
    ...
    if (!body.id)
      return res.status(400).json({ message: "Incomplete parameters" });
    try {
      const data = await EvalQuestions.destroy({
        where: { id: body.id },
      });
